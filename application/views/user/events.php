...
<head>
    ...
    <title>Upcoming Event Feed: Modernized</title>
...
<body class="text-slate-800 antialiased">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-1">Explore and register for our exciting workshops, hackathons, and networking sessions.</p>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-5 mb-10">
                ...
                <div class="col-span-2 relative">
                    ...
                    <input type="text" id="search-input" placeholder="Search event name or topics..." class="pl-10 w-full p-3 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-secondary focus:border-secondary block" onkeyup="renderEvents()">
                </div>

                <div>
                    <select id="filter-type" class="w-full p-3 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-secondary focus:border-secondary" onchange="renderEvents()">
                ...
                <div>
                    <select id="sort-order" class="w-full p-3 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-secondary focus:border-secondary" onchange="renderEvents()">
        ...
        <div id="no-events-message" class="hidden text-center py-20 bg-white rounded-xl shadow-lg border border-gray-100">
            ...
            <h3 class="text-xl font-bold text-gray-900">No Upcoming Events Found</h3>
            <p class="text-gray-500 mt-2">The current filters returned no matching future events.</p>
    ...
    <script>
        // --- 1. Enhanced Data Source (Updated dates to future, added 'conference' type, and more events) ---
       // --- 1. Enhanced Data Source (Updated dates to future, added 'conference' type, and more events) ---
        ...
        function timeUntil(dateString) {
            ...
            if (seconds < 0) return "Event Passed";
        ...
        // Reused timeAgo from original for "posted" date in modal
        ...
        function getEventStyle(type) {
            const styles = {
                ...
                // New Type Style
                conference: { 
                    bg: "bg-red-100", text: "text-red-800", border: "border-red-200", ring: "ring-red-500",
        ...
        function renderEvents() {
            ...
            // 1. Base Filter: FUTURE events only
            ...
            // 4. Sorting (Closest First is Ascending Date)
            results.sort((a, b) => {
            ...
            modalContainer.innerHTML = ''; // Recreate modals for dynamic data
            counterElement.textContent = `${results.length} Upcoming Event${results.length !== 1 ? 's' : ''}`;
                        ...
                        <div class="fade-in-up group bg-white rounded-xl border border-gray-200 shadow-md hover:shadow-xl transition-all duration-500 overflow-hidden flex flex-col h-full hover:ring-2 ${style.ring}/50 cursor-pointer" onclick="openModal(${event.id})" style="animation-delay: ${delay}ms">
                            ...
                            <div class="p-6 flex-grow">
                                <div class="flex justify-between items-center mb-3">
                                    ...
                                    <span class="text-xs font-bold text-red-600 bg-red-50 px-2 py-0.5 rounded-full border border-red-200">
                                        Starts in ${timeUntil(event.event_date)}
                                    </span>
                                </div>
                            ...
                            <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
                                ...
                                <a href="#" class="text-sm font-semibold text-white bg-primary hover:bg-blue-800 rounded-lg px-3 py-1.5 transition shadow-sm">
                                    Register <span aria-hidden="true" class="ml-1 text-lg leading-none align-middle inline-block">→</span>
                                </a>
                            </div>
                        </div>
                    `;
                    ...
                    <div class="fixed inset-0 z-10 overflow-y-auto">
                        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                            <div class="relative transform rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-xl scale-95 opacity-0 modal-panel">
                                ...
                                <div class="bg-gray-50 px-6 py-4 sm:flex sm:flex-row-reverse sm:px-6 border-t border-gray-100 rounded-b-2xl">
                                    <a href="#" class="mt-3 inline-flex w-full justify-center rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-white shadow-md hover:bg-blue-800 transition sm:mt-0 sm:ml-3 sm:w-auto" target="_blank">Register Now</a>
                                    <button type="button" class="inline-flex w-full justify-center rounded-lg bg-white px-4 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:w-auto" onclick="closeModal(${event.id})">Close</button>
                                </div>
                            </div>
                        </div>
                    </div>
